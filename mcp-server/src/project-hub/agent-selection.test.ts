...
const capabilities = [{ capability: 'workflow.recovery.apply', state: 'available' as const }];
...
test('Agent Domain selection keeps only enabled exact Project/Profile/capability tuples', () => {
  const result = normalizeCompatibleBindings('project/alpha', capabilities, [
    { role: 'builder', bindingId: 'binding/alpha/builder', bindingRevision: 2, profileId: 'agent/builder', profileRevision: 3, projectId: 'project/alpha', capabilityClaims: ['workflow.recovery.apply'] },
