...
import { formatLastUsed, getSkillTraces, runSkillkitActionArgs, isSkillkitAvailable } from "../skillkit";
...
export class DetailPanel {
	...
	clear(): void {
      ...
      empty.createDiv({ text: "Select a capability to view" });
	}
		...
		if (isSkillkitAvailable()) {
			...
			deleteBtn.addEventListener("click", () => {
				showConfirmModal(this.app, "Remove skill", `Remove "${item.name}"? This will delete the skill files.`, () => {
					const result = runSkillkitActionArgs(["prune", "--skill", item.name, "--yes"]);
					if (result.success) {
