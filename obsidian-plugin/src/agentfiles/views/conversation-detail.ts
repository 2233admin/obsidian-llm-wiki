...
import { generateNoteContent, stripNoteFrontmatter } from "../conversations/note-exporter";
import type { AgentfilesDraftWriter } from "../feature";
...
export class ConversationDetailPanel {
	...
	private writeInboxDraft?: AgentfilesDraftWriter;
	...
	constructor(
		...
		app: App,
		vaultPath: string,
		writeInboxDraft?: AgentfilesDraftWriter,
	) {
		this.containerEl = containerEl;
		this.store = store;
		this.app = app;
		this.vaultPath = vaultPath;
		this.writeInboxDraft = writeInboxDraft;
	}
	...
	private async saveToVault(item: ConversationItem): Promise<void> {
		if (this.selectedMessages.size === 0) {
			new Notice("Select messages to export first", 3000);
			return;
		}
		if (!this.writeInboxDraft) {
			new Notice("Inbox export is unavailable until the LLM Wiki control plane is ready", 5000);
			return;
		}
		...
		try {
			const result = await this.writeInboxDraft({
			parentQuery: `Claude session: ${item.title}`,
			sourceNodes: [`conversation:${item.uuid}`],
				body: stripNoteFrontmatter(content),
			slug: `conversation-${item.uuid}`,
		});
			new Notice(`Saved to ${result.path}`, 5000);
		} catch (e: unknown) {
