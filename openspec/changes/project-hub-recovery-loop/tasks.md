...
- [ ] **T5.3 Verify S06A in actual Obsidian** — **Unblocked by verified S04P.** Then run plugin tests/typecheck/build, exercise all six read-only stages (`open`, `searched`, `needs-agent-selection` with valid multiple Bindings/candidates and no generated Plan request, `planned` with plan available while apply absent/unusable, `stale`, `unavailable`), prove fixture vault/plugin/durable-root SHA-256 and byte equality before/after each stage, reload the ItemView to prove Flow state disappears, and record interaction/visual evidence. S04B remains blocked until principal accepts the actual surface.
