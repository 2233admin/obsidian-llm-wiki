...
- [ ] **T4.5.1 Register planning Operation and shared service (R12, R13, R15)** — Construct `RecoveryPlanningService` once in core composition and inject it into both `makeProjectHubOps` and `makeWorkflowOps`. Register `workflow.recovery.plan` with `mutating: false`, request schema `workflow-recovery-plan-request/v1` (outer envelope containing one `project-hub-recovery-flow-request/v2` Plan arm), response stages `planned|stale|unavailable` (V2 Flow response schemas). The Operation delegates plan/refresh/override actions to the shared service; semantics, validators, and fingerprints are not duplicated. Schema vocabulary: no `workflow-recovery-plan/v1` Plan or response schema.
- [ ] **T4.5.2 Introduce planning capability fact (R14, R15)** — After Operation registration, `workflow.recovery.plan` capability enters `available` state. Add capability fact injection to the same factory used by other Workflow Operations. Operation order does not determine capability visibility. Add integration test: capability appears after registration even if `makeProjectHubOps` is not yet called.
- [ ] **T4.5.3 Update candidate composition (R12, R14)** — Modify `composeRecoveryCandidates` to check `workflow.recovery.plan: available` instead of `workflow.recovery.apply: available` for candidate recommendation. Missing planning capability produces `capability_unavailable` with bounded remediation. Update `action-candidates.test.ts` capability fixture from `workflow.recovery.apply` to `workflow.recovery.plan`.
...
- [ ] **T4.5.5 Verify S04P** — Run focused Operation registration test, capability fact test, candidate composition test, and Agent Domain Profile test. Add integration tests proving: (a) Project Hub sees `workflow.recovery.plan` after registration; (b) Project Hub never sees `workflow.recovery.apply` before S04B; (c) apply request returns `unavailable` before S04B. Record S04P evidence in `p0-s04p-workflow-recovery-plan.md`; S06A T5.3 resumes after principal accepts this work.
...
- [ ] **T5.3 Verify S06A in actual Obsidian** — Run plugin tests/typecheck/build, prove no fixture bytes or plugin data change, reload the ItemView to prove Flow state disappears, and record interaction/visual evidence. **This task is proposed next** in the dependency chain after S04P lands; S04B remains blocked until principal accepts the actual surface.
