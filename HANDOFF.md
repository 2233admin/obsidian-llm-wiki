...
4. **Task 12 — Context Core**(草案 `f407dff`,`TASK12-DRAFT-context-core.md`):内化 TrustGraph 的"Context Core = 知识作为可版本化便携工件"概念,取概念丢运行时(git=版本/pin/promote,markdown 真值,无 DB)。净新:`_context-core.json` 编译 + retrieval-policy 一等物 + explainability receipt + (可选)ontology 存量 backfill 治空转。建造序 12A(core 编译,纯 Python,先做)→12B→12C→12D(11G 重构成从 core 取)。**未建**,待开 ADR(core git-tracked vs gitignore)。
