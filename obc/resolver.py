...
class Resolver:
    ...
    def _resolve_wikilink(self, link: LinkRef) -> Diagnostic:
        ...
        target = link.target_path_part or link.target_raw
        ...
        # 2. Try unique basename match (if stem didn't work)
        candidates = self.index.get_by_basename(Path(target).name)
        if len(candidates) == 1:
            return Diagnostic(
                ...
                suggested_fix=f"[[{candidates[0].normalized_path}]]",
                safety_level="S2",
            )
        elif len(candidates) > 1:
            return Diagnostic(
        ...
        # 3. Try alias match (as suggestion only)
        if self.config.resolve_aliases != "ignore":
            alias_candidates = self.index.get_by_alias(target)
            if alias_candidates:
                return Diagnostic(
                    code=DiagnosticCode.AMBIGUOUS_TARGET,
                    link=link,
                    candidates=alias_candidates,
                    message=f"Alias match found: {[c.normalized_path for c in alias_candidates]}",
                    suggested_fix=self._make_alias_suggestion(link, alias_candidates[0]),
                    safety_level="S2",
                )
    ...
    def _make_alias_suggestion(self, link: LinkRef, target: FileEntry) -> str:
        """Make a safe fix suggestion that preserves alias."""
        if link.alias:
            return f"[[{target.normalized_path}|{link.alias}]]"
        elif link.fragment:
            return f"[[{target.normalized_path}#{link.fragment}]]"
        else:
            return f"[[{target.normalized_path}]]"
