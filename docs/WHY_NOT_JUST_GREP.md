...
Grep scans on every invocation. The first query costs the same as the 1,000th. LLM Wiki compiles your vault once. On a 554-note vault, it produces a concept graph with 2,507 edges in under 3 seconds. Every subsequent query reuses that graph. The agent calls `vault.backlinks` and gets the full backlink map without rescanning the filesystem.
