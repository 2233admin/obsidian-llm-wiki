...
Graphiti supports richer ontology patterns, but LLMwiki's first graph-index slice needs to prove rebuildability, provenance, freshness, and query integration. A broad ontology surface would expand the product before the derived-index boundary is proven.
...
Missing validity windows are allowed. `observed_at` records when LLMwiki saw the evidence; `valid_from` and `valid_to` record when the fact was true in the world only when evidence supports that window. `temporal_status` distinguishes `atemporal`, `inferred_window`, `explicit_window`, and `unknown`.
